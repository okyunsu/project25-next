 ...
 * 
 * @returns 폼 상태, 에러 메시지, 이벤트 핸들러를 포함한 객체
 */
export function useSignup() {
  ...
  const validateForm = (): boolean => {
    // 모든 필드 입력 확인
    if (!formData.user_id || !formData.email || !formData.password || !formData.confirmPassword || !formData.name) {
